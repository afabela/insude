...
function ParticipantesNuevoCtrl($scope, $meteor, $reactive, $state, toastr, $stateParams) {
	...
	*/
	...
	this.subscribe('eventosComposite', () => {
	...
	*/
	...
	/*
		this.subscribe('entrenadores',()=>{
			return [{evento_id:  this.getReactively('participanteEventos.evento_id') 
	//						,deporte_id: this.getReactively('participanteEventos.deporte_id')
							,funcionEspecifica : "ENTRENADOR"
			}]
		});	
	*/
	...
	this.helpers({
		...
		categorias: () => {
			return Categorias.find({ deporte_id: this.getReactively('participanteEventos.deporte_id') });
		},
		pruebas: () => {
			return Pruebas.find({ categoria_id: this.getReactively('participanteEventos.categoria_id') });
		},
		ramas: () => {
			return Ramas.find();
		},
  ...
  */
		...
		/*
			  entrenadores : () => {
				  return ParticipanteEventos.find({}, { sort : {"nombreCompleto" : 1 }})
				},
		*/
		...
		if (participanteEventos.categoria_id != "s/a") {
			...
			if (participanteEventos.funcionEspecifica != 'DEPORTISTA') {
				...
				Meteor.call('getParticipanteCurp', participante.curp, function (error, response) {
					...
					//Ya Existe
					if (response) {
						//Preguntar si es del mismo monicipio sino mostar que municipio esta
						if (Meteor.user().profile.municipio_id != response.municipio_id) {
							Meteor.call('getMunicipio', response.municipio_id, function (error, result) {
								if (result) {
									toastr.warning("El Participante ya se encuentra registrado en " + result);
							...
							loading(true);
							Meteor.call('updateParticipanteEventos', participante, participanteEventos, $stateParams.id, function (error, result) {
								if (error) {
									console.log('ERROR :', error);
									loading(false);
									return;
				...
				Meteor.call('getParticipanteCurp', participante.curp, function (error, response) {
					...
					if (response) {
						...
						if (Meteor.user().profile.municipio_id != response.municipio_id) {
							Meteor.call('getMunicipio', response.municipio_id, function (error, result) {
								if (result) {
									toastr.warning("El Deportista ya se encuentra registrado en " + result);
									$("#registrar").prop("disabled", false);
								}
							});
						}
			...
			Meteor.call('getParticipanteCurp', participante.curp, function (error, response) {
				...
				if (response) {
					loading(true);
					Meteor.call('updateParticipanteEventos', participante, participanteEventos, $stateParams.id, function (error, response) {
						if (error) {
							console.log('ERROR :', error);
							loading(false);
							return;
